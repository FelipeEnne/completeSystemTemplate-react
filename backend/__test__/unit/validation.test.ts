const app_test_validation = require("../../index.ts");

describe("Valitador the communication inputs (existsOrError)", () => {
  const { existsOrError } = app_test_validation.api.validation;
...
describe("Valitador the communication date", () => {
  const { verifyDeliveryDate, verifyIfDeliveryDatePast } =
    app_test_validation.api.validation;
  const errorCommunication = "Error";
...
describe("Valitador the communication format", () => {
  const { verifyValidCommunicationFormat } = app_test_validation.api.validation;
  const errorCommunication = "Error";
